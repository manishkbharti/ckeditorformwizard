...
$(document).ready(function () {
    ...
    if($(".username-check").length > 0){
        ...
        $(".username-check").change(function(e){
            ...
            $.post("/register/checkUserAvailability",{username: $this.val()}, function(e){
                if(e.available == "true"){
...
$(".tasklist").on('click', '.task-bookmark', function(e){
    ...
    var taskId=$(this).parent().parent().find("[class='taskId']").text();
...
$(".new-task-form").submit(function(e){
    ...
    var $form = $(this),
        $tasklist = $(".tasklist");
    var $icon = $form.find("select[name=icon]"),
        $name = $form.find("input[name=description]"),
        $bookmark = $form.find("input[name=important]");
    $.ajax({
        url:'/task/addTask',
        data: $(this).serialize(),
